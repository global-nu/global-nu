...
FIELD_DOCS = [
    ...
    # Deliberately short. The propagation formula, the sign convention and
    # what omitting rho costs all live in the prose on history.html, where
    # there is room for them; a column description that runs five times longer
    # than every other row stops being a table anyone reads.
    ("interval_method", "How the converted interval was obtained. "
                        "<code>identical</code>: not converted. "
                        "<code>propagated</code>: centre shifted by "
                        "&delta;m&sup2;/2, and &sigma;(&delta;m&sup2;/2) added "
                        "in quadrature to each half-width. "
                        "<code>propagated_rho</code>: the same "
                        "with the &delta;m&sup2;&ndash;&Delta;m&sup2; correlation "
                        "included, which no release here carries yet. The formula "
                        "is on the parameter-history page. Empty for a "
                        "<code>limit</code> row"),
]
